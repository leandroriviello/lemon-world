...
const Header = (props: { children: ReactNode; className?: string }) => {
  return (
    <header
      className={twMerge(
        'bg-background flex flex-col justify-center px-6 pt-6 pb-3 z-10 border-b border-border',
...
const Main = (props: { children: ReactNode; className?: string }) => {
  return (
    <main
      className={twMerge(
        clsx('grow overflow-y-auto p-6 pt-3 bg-background', props.className),
      )}
    >
      {props.children}
    </main>
...
const Footer = (props: { children: ReactNode; className?: string }) => {
  return (
    <footer className={twMerge('px-6 pb-[35px] bg-background border-t border-border', clsx(props.className))}>
      {props.children}
    </footer>
  );
};
